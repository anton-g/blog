...
const EggMask = () => {
  return (
    <defs>
      <mask id="eggMask">
        <svg width="100%" height="100%" viewBox="0 0 246 371" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
  ...
  return (
    <>
      {coordinates.map((coord) => (
        <SmallBlock key={`${coord.x}${coord.y}}`} x={coord.x} y={coord.y} colors={colors} />
      ))}
    </>
  )
}
...
const SmallBlock = ({ x, y, colors }: SmallBlockProps) => {
  ...
  return (
    <motion.g
      ...
      transition={{
        type: 'spring',
        delay: random(1, animationTime),
      }}
    >
      <rect height={squareSize} width={squareSize} fill={background} x={x} y={y} />
      <Decoration x={x} y={y} foreground={foreground} background={background} squareSize={squareSize} />
    </motion.g>
  )
}
...
const BigBlock = ({ colors, numRows, numCols }: BigBlockProps) => {
  ...
  const Decoration = random([HalfSquare, DiagonalSquare, QuarterCircle, OppositeCircles, Circle, Cross, Dots])

  const multiplier = random([2, 3, 4])
  ...
  return (
    ...
    >
      <rect height={bigSquareSize} width={bigSquareSize} fill={background} x={x} y={y} />
      <Decoration x={x} y={y} foreground={foreground} background={background} squareSize={bigSquareSize} />
    </motion.g>
  )
}
...
const Circle = ({ x, y, foreground, background, squareSize }: DecorationProps) => {
  const circleRadius = squareSize / 2

  const showInner = random(0, 1) < 0.3
  return (
    <g>
      <circle r={circleRadius} fill={foreground} cx={x + circleRadius} cy={y + circleRadius} />
      {showInner && <circle r={circleRadius / 2} fill={background} cx={x + circleRadius} cy={y + circleRadius} />}
    </g>
  )
}
...
const Cross = ({ x, y, foreground, squareSize }: DecorationProps) => {
  ...
  return (
    <g>
      <rect width={width} height={height} fill={foreground} x={x + width / 4} y={y + height * 2} />
      <rect
        width={width}
        height={height}
        fill={foreground}
  ...
  return (
    <g>
      {dotCoordinates.map((coord) => (
        <circle key={`${coord.x}${coord.y}`} r={circleSize / 2} cx={coord.x} cy={coord.y} fill={foreground} />
      ))}
    </g>
  )
}
...
const OppositeCircles = ({ x, y, foreground, squareSize }: DecorationProps) => {
  ...
  return (
    <>
      ...
      <g mask={`url('#${maskId}'`}>
        <circle r={circleRadius} fill={foreground} cx={positions[0]} cy={positions[1]} />
        <circle r={circleRadius} fill={foreground} cx={positions[2]} cy={positions[3]} />
      </g>
    </>
  )
}

const QuarterCircle = ({ x, y, foreground, background, squareSize }: DecorationProps) => {
  const id = useId()
  const maskId = `mask-${id}`
  ...
  return (
    <>
      ...
      <g mask={`url(#${maskId})`}>
        <circle r={circleRadius} fill={foreground} cx={x + xOffset} cy={y + yOffset} />
        {showInner && <circle r={circleRadius / 2} fill={background} cx={x + xOffset} cy={y + yOffset} />}
      </g>
    </>
  )
}
...
const DiagonalSquare = ({ x, y, foreground, squareSize }: DecorationProps) => {
  const rightLeaning = `${x},${y} ${x},${y + squareSize}, ${x + squareSize},${y}`
  const leftLeaning = `${x},${y} ${x + squareSize},${y} ${x + squareSize},${y + squareSize}`
...
const HalfSquare = ({ x, y, foreground, squareSize }: DecorationProps) => {
  ...
  return (
    <g>
      <rect width={squareSize / halfX} height={squareSize / halfY} fill={foreground} x={x} y={y} />
    </g>
  )
}

const SquareMask = ({ id, x, y, squareSize }: { id: string; x: number; y: number; squareSize: number }) => {
  return (
    <mask id={id}>
