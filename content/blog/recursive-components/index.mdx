...
So a recursive function is a function that invokes itself. In the case of traversing the above tree we would have a function `traverse(node)` that we'd invoke for the root of the tree, and then it invoke itself for every child. So first `traverse(A)`, then `traverse(B)` and then those would invoke the function on their children and so on.
...
The only important part of recursion we need to keep track of in our implementation of a recursive component is **the base case**. This is the case when we need to break out of the recursion and stop rendering ourself, otherwise we'd be stuck in an infinite loop.
...
The one thing missing from this is that we need to identity our base case so that we can break out of the recursion. In our case this will be when the folder has no children, or in other words, when it's a _leaf_ in our tree. So let's add a conditional that checks just this.
