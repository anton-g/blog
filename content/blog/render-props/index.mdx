...
date: '2020-05-16T14:17:00.000Z'
description: "Let's take a look at what render props is, and what use case they have now that we have hooks."
dev: ''
draft: true
---
...
Through the years many different patterns have emerged to solve problems we encounter writing React components. One of the most popular patterns ever is the `render prop`-pattern. A render prop is a way to solve a problem where you want to share common functionality.
...
One of the most straight forward use cases I've found for render props nowadays is to wrap hooks to use them in class components. Let's say we've previously used a render prop to track whether the mouse is hovering an element and now we refactor this to a [`useHover`](https://usehooks.com/useHover/) hook instead. To use this in a class component we can wrap it in a render prop:

```jsx
const Hover = ({ children }) => children(useHover())
```

And then we can use it just like we would if `Hover` took care of the implementation itself:
...
Now imagine that we need to change how a row or cell is displayed. Here we could implement two render props in the component that defers this rendering to the user:
...
Now when we hover the only thing that's gonna rerender is the `div`! I do think this is maybe the most opinionated use of the render prop pattern, and personally I can't really decide wether I prefer this over breaking it out to another component. Choice is always good though!

## Summary

While Hooks have taken over a lot of the responsibility of render props they should still have a seat among the patterns we use when solving problems with React, just as long as we use them for the right reasons.
